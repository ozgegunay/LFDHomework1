...
def learn(trainingSet, trainingClassification):
    iteration = 1
    return iteration, np.matmul(np.linalg.pinv(trainingSet), trainingClassification)
...
for i in range(1000):
    weight = [0, 0, 0]
    f = target()
    trainingInput = [[1.0, random.uniform(-1,1), random.uniform(-1,1)] for i in range(10)]
    trainingClassification = [compareResult(f(x[1]), x[2]) for x in trainingInput]
    iteration, weight = learn(trainingInput, trainingClassification)
